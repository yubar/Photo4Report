...
def main():
	...
	#path = u"."
	path = u"d:\\prvt\\photo\\202207 Путораны" 
	#outpath = u".\\out"
	outpath = u"d:\\prvt\photo\\202207 Путораны mpl tst\\out"
	ext = u"jpg"
	...
	trackFileName = u'Fact_raw.gpx'
	
	#delta = timedelta(seconds=0)
	#tzGMT = -timedelta(hours=12)
	#tzLocal = timedelta(hours=0)
	
	delta = timedelta(minutes=7, seconds=35)
	tzGMT = -timedelta(hours=3)
	tzLocal = timedelta(hours=4)
	...
	for file in glob.glob("*." + ext):
		...
		#if (re.match('^[\d]{8}_[\d]{6}_', file) or re.match('^[\d]{2}_[\d]{8}_[\d]{6}_', file)): continue
		...
		#logging.info(file + ' ' + str(d) + ' ' + str(dtupd) + ' ' + outname)
